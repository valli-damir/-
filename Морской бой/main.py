...
class Board:
    ...
    def __str__(self):
        ...
        return res
    def out(self, out_):
        return not ((0 <= out_.x < self.size) and (0 <= out_.y < self.size))
    def add_ship(self, ship):
        for out_ in ship.dots:
            if self.out(out_) or out_ in self.busy:
                raise BoardWrongShipException()
        for out_ in ship.dots:
            self.field[out_.x][out_.y] = "■"
            self.busy.append(out_)
    ...
    def contour(self, ship, verb = False):
        ...
        for out_ in ship.dots:
            for out_x, out_y in near:
                cur = Dot(out_x + out_.x, out_y + out_.y)
    ...
    def shot(self, out_):
        ...
        if out_ in self.busy:
            raise BoardUsedException()
        self.busy.append(out_)

        for ship in self.ships:
            if ship.shooten(out_):
                ship.lives -= 1
                self.field[out_.x][out_.y] = "X"
                if ship.lives == 0:
                    self.count += 1
